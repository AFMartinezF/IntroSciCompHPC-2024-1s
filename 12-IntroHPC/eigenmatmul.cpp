...
/* Ciclo en bash probando con distinto número de hilos: 
    for nth in {1..4}; do echo -n "$nth  "; OMP_NUM_THREADS=$nth  ./a.out 3000 2 2>/dev/null; done
   Con tee podemos escribir el archivo mientras vemos el resultado en pantalla:
    for nth in {1..4}; do echo -n "$nth  "; OMP_NUM_THREADS=$nth  ./a.out 3000 2 2>/dev/null; done | tee times.txt

   Calculo del SpeedUp
    awk '{print $1,3.32296/$3}' times.txt donde 3.32296 es el tiempo de ejecución con un solo hilo
   Añadimos el calculo de la eficiencia en una nueva columna
    awk '{print $1,3.32296/$3, 3.32296/$3/$1}' times.txt > metrics.txt
*/